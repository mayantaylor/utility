...
{
	return (xdr_template(xdrs, &(val->time)) && xdr_template(xdrs, &(val->nbodies)) && xdr_template(xdrs, &(val->ndim)) && xdr_template(xdrs, &(val->nsph)) && xdr_template(xdrs, &(val->ndark)) && xdr_template(xdrs, &(val->nstar)));
}

/// XDR conversions for the particle types
inline bool_t xdr_template(XDR *xdrs, Tipsy::simple_particle *p)
{
	return (xdr_template(xdrs, &(p->mass)) && xdr_template(xdrs, &(p->pos)) && xdr_template(xdrs, &(p->vel)));
}

template <typename TPos, typename TVel>
inline bool_t xdr_template(XDR *xdrs, Tipsy::gas_particle_t<TPos, TVel> *p)
{
	return (xdr_template(xdrs, &(p->mass)) && xdr_template(xdrs, &(p->pos)) && xdr_template(xdrs, &(p->vel)) && xdr_template(xdrs, &(p->rho)) && xdr_template(xdrs, &(p->temp)) && xdr_template(xdrs, &(p->hsmooth)) && xdr_template(xdrs, &(p->metals)) && xdr_template(xdrs, &(p->phi)));
}

template <typename TPos, typename TVel>
inline bool_t xdr_template(XDR *xdrs, Tipsy::dark_particle_t<TPos, TVel> *p)
{
	return (xdr_template(xdrs, &(p->mass)) && xdr_template(xdrs, &(p->pos)) && xdr_template(xdrs, &(p->vel)) && xdr_template(xdrs, &(p->eps)) && xdr_template(xdrs, &(p->phi)));
}

template <typename TPos, typename TVel>
inline bool_t xdr_template(XDR *xdrs, Tipsy::star_particle_t<TPos, TVel> *p)
{
	return (xdr_template(xdrs, &(p->mass)) && xdr_template(xdrs, &(p->pos)) && xdr_template(xdrs, &(p->vel)) && xdr_template(xdrs, &(p->metals)) && xdr_template(xdrs, &(p->tform)) && xdr_template(xdrs, &(p->eps)) && xdr_template(xdrs, &(p->phi)));
}
...
namespace Tipsy
{
	...
	bool TipsyReader::loadHeader()
	{
		...
		{ // perhaps it's XDR
			...
			if (!xdr_template(&xdrs, &h) || h.ndim != MAXDIM)
			{ // wasn't xdr format either
	...
	bool TipsyReader::getNextDarkParticle_t(dark_particle_t<TPos, TVel> &p)
	{
		...
		if (numGasRead != h.nsph)
		{ // some gas still not read, skip them
	...
	bool TipsyReader::getNextStarParticle_t(star_particle_t<TPos, TVel> &p)
	{
		...
		if (numGasRead != h.nsph || numDarksRead != h.ndark)
		{ // some gas and dark still not read, skip them
